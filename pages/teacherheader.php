...
<div class="collapse shadow my-4" id="collapseExample" style="position: absolute;top:80px;right:200px;width:500px;z-index:999">
  <div class="card card-body bg-white ">
    ...
    <?php if ($teachers) { ?>
      ...
      <span>
        <a href="teacherprofile.php" class="text-secondary small"><i class="fas fa-pen"></i>&nbsp; Change profile</a>
      </span>
      ...
      <span>
        <span class="float-left font-weight-bold px-2">
          <?php
          $teacherData = $obj->Select('tbl_teacher', '*', 'tname', array($_SESSION['posted_by']));
          if ($teacherData) {  ?>
            <?php foreach ($teacherData as $key => $value) : ?>
              Subject : <?php
                        $subj = $obj->select("addsubject", "*", "sub_id", array($value['tsubject']));
                        echo $subj[0]['subjectname'];
                        $sem_str = $obj->select("semesters", "*", "id", array($value['tsemester']));
                        $sem  =  $sem_str[0]['name'];
